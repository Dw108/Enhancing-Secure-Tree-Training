...
def sub_feature(name, arr_train, arr_validate, arr_test, scale1, scale2):
    ...
    name_feature.insert(3, 'F2_sub')
    name_feature.insert(6, 'F4_sub')
    train_sub_feature_1 = []
    validate_sub_feature_1 = []
    test_sub_feature_1 = []
    for i in range(len(arr_train)):
        tmp = arr_train[i].astype(float)
        tmp = np.insert(tmp, 3, float(tmp[2]) + np.random.laplace(loc=0, scale=scale1))
        tmp = np.insert(tmp, 6, float(tmp[5]) + np.random.laplace(loc=0, scale=scale2))
    ...
    # 分裂属性
    # 未加密
    write_data("../../../dataset/bias/CASP/CASP_sub_feature_train" + str(scale1) + '_' + str(scale2) + ".csv",
               name_feature, train_sub_feature_1)

    write_data("../../../dataset/bias/CASP/CASP_sub_feature_validate" + str(scale1) + '_' + str(scale2) + ".csv",
               name_feature, validate_sub_feature_1)
    write_data("../../../dataset/bias/CASP/CASP_sub_feature_test" + str(scale1) + '_' + str(scale2) + ".csv",
               name_feature, test_sub_feature_1)
    # 加密
    uni_data_1_1 = cols_enc([train_sub_feature_1, validate_sub_feature_1, test_sub_feature_1])
    write_data("../../../dataset/bias/CASP/CASP_sub_feature_train_enc" + str(scale1) + '_' + str(scale2) + ".csv",
               name_feature, uni_data_1_1[0])
    write_data("../../../dataset/bias/CASP/CASP_sub_feature_validate_enc" + str(scale1) + '_' + str(scale2) + ".csv",
               name_feature, uni_data_1_1[1])
    write_data("../../../dataset/bias/CASP/CASP_sub_feature_test_enc" + str(scale1) + '_' + str(scale2) + ".csv",
...
def sub_item(name, arr_train, arr_validate, arr_test, scale1, scale2):
    ...
    for i in range(len(arr_train)):
        ...
        tmp[2] = float(tmp[2]) + np.random.laplace(loc=0, scale=scale1)
    ...
    for i in range(len(arr_train)):
        tmp = copy.deepcopy(arr_train[i])
        tmp[4] = float(tmp[4]) + np.random.laplace(loc=0, scale=scale2)
        arr_train.append(tmp)
    ...
    # 分裂条目
    # 未加密
    write_data("../../../dataset/bias/CASP/CASP_sub_item_train" + str(scale1) + '_' + str(scale2) + ".csv", name, arr_train_tmp)
    write_data("../../../dataset/bias/CASP/CASP_sub_item_validate" + str(scale1) + '_' + str(scale2) + ".csv", name, arr_validate_tmp)
    write_data("../../../dataset/bias/CASP/CASP_sub_item_test" + str(scale1) + '_' + str(scale2) + ".csv", name, arr_test_tmp)

    # 加密
    uni_data_2 = cols_enc([arr_train_tmp, arr_validate_tmp, arr_test_tmp])
    write_data("../../../dataset/bias/CASP/CASP_sub_item_train_enc" + str(scale1) + '_' + str(scale2) + ".csv", name, uni_data_2[0])
    write_data("../../../dataset/bias/CASP/CASP_sub_item_validate_enc" + str(scale1) + '_' + str(scale2) + ".csv", name, uni_data_2[1])
    write_data("../../../dataset/bias/CASP/CASP_sub_item_test_enc" + str(scale1) + '_' + str(scale2) + ".csv", name, uni_data_2[2])
def cols_enc(data):
    # data全维度数据
    cols_train = []
    cols_val = []
...
def main():
    ...
    # 无重叠数据
    # 未加密
    write_data("../../../dataset/bias/CASP/CASP_train.csv", name, arr_train)
    write_data("../../../dataset/bias/CASP/CASP_validate.csv", name, arr_validate)
    write_data("../../../dataset/bias/CASP/CASP_validate.csv", name, arr_test)
    # 无重叠 加密
    uni_data_0 = cols_enc([arr_train, arr_validate, arr_test])
    write_data("../../../dataset/bias/CASP/CASP_train_enc.csv", name, uni_data_0[0])
    write_data("../../../dataset/bias/CASP/CASP_validate_enc.csv", name, uni_data_0[1])
    write_data("../../../dataset/bias/CASP/CASP_test_enc.csv", name, uni_data_0[2])

    # 创建子属性的方式
    sub_feature(name, arr_train, arr_validate, arr_test, scale1=1000 / 1, scale2=100 / 1)
