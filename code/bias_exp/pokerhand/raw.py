...
# 读取数据
# 分裂属性的数据
# df = read_csv("../../../dataset/test/poker_train.csv")
df = read_csv("../../../dataset/test/poker_train_enc.csv")
data = df.values
X_train_feature = data[:, 1:11].astype(int)
X_train_feature_ = data[:, 0:1].astype(int)
y_train_feature = data[:, -1].astype(int)

# df = read_csv("../../../dataset/test/poker_validate.csv")
df = read_csv("../../../dataset/test/poker_validate_enc.csv")
data = df.values
X_validate_feature = data[:, 1:11].astype(int)
X_validate_feature_ = data[:, 0:1].astype(int)
y_validate_feature = data[:, -1].astype(int)

# df = read_csv("../../../dataset/test/poker_test.csv")
df = read_csv("../../../dataset/test/poker_test_enc.csv")
data = df.values
X_test_feature = data[:, 1:11].astype(int)
