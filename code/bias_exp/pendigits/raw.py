...
# 读取数据
# 分裂属性的数据
df = read_csv("../../../dataset/test/pen_train.csv")
data = df.values
X_train_feature = data[:, 1:17].astype(int)
X_train_feature_ = data[:, 0:1].astype(int)
y_train_feature = data[:, -1].astype(int)

df = read_csv("../../../dataset/test/pen_validate.csv")
data = df.values
X_validate_feature = data[:, 1:17].astype(int)
X_validate_feature_ = data[:, 0:1].astype(int)
y_validate_feature = data[:, -1].astype(int)

df = read_csv("../../../dataset/test/pen_test.csv")
data = df.values
X_test_feature = data[:, 1:17].astype(int)
