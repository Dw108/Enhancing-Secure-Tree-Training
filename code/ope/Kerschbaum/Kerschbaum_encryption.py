...
def encrypt(encrypt_tree, dataset, min_value, max_value):
...
def encrypt_uni(encrypt_tree, dataset, min_value, max_value):

    _, sorted_plaintexts, sorted_ciphertexts, _ = encrypt(encrypt_tree, dataset, min_value, max_value)
    tmp = []
    max=0
    for i in dataset:
        if max % 10000 == 0:
            print(max)
        idx = sorted_plaintexts.index(i)
        count = sorted_plaintexts.count(i)
        tmp_index = random.randint(idx, idx + count -1)
        tmp.append(sorted_ciphertexts[tmp_index])
        max = max+1
    return tmp
...
# A = [1, 22, 64998, 1]
# encrypt_tree = Tree()
# print(encrypt_uni(encrypt_tree, A, 0, 100000))
