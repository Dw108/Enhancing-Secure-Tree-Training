...
X_feature_second_train = X_feature[0:share_size * 6, :]
y_feature_second_train = y_feature[0:share_size * 6]

X_item_train = np.concatenate((X_item[0:share_size * 1, :], X_item[share_size * 2:share_size * 6, :]))
X_item_val = X_item[share_size * 1:share_size * 2, :]
X_item_test = X_item[share_size * 6:, :]
y_item_train = np.concatenate((y_item[0:share_size * 1], y_item[share_size * 2:share_size * 6]))
y_item_val = y_item[share_size * 1:share_size * 2]
y_item_test = y_item[share_size * 6:]

X_item_second_train = X_item[0:share_size * 6, :]
y_item_second_train = y_item[0:share_size * 6]

X_select_train = X_feature[share_size * 1:share_size * 6, :]
X_select_val = X_feature[share_size * 0:share_size * 1, :]
X_select_test = X_feature[share_size * 6:, :]
y_select_train = y_feature[share_size * 1:share_size * 6]
y_select_val = y_feature[share_size * 0:share_size * 1]
y_select_test = y_feature[share_size * 6:]

X_select_second_train = X_feature[0:share_size * 6, :]
y_select_second_train = y_feature[0:share_size * 6]
...
for thresh in thresholds:
    ...
    # train model
    selection_model = XGBClassifier(n_estimators=n_estimators, max_depth=max_depth, learning_rate=1,
