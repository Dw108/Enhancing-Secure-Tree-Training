...
write_data("../../../dataset/overlappingnums/covtype/mid/label_feature_train.csv", ['label'],
           np.array([y_feature_train]).T)
...
for i in range(2, 10):
    ...
    acc_feature = accuracy_score(y_feature_val_tmp, bst_feature.predict(X_feature_val_tmp))
    acc_feature_test = accuracy_score(y_feature_test_tmp, bst_feature.predict(X_feature_test_tmp))
    accumulation_train = bst_feature.predict(X_feature_train_tmp, output_margin=True)
    accumulation_val = bst_feature.predict(X_feature_val_tmp, output_margin=True)
    accumulation_test = bst_feature.predict(X_feature_test_tmp, output_margin=True)
    name_feature =['1', '2', '3', '4', '5', '6', '7']
    write_data("../../../dataset/overlappingnums/covtype/mid/data_feature_train_" + str(i) + ".csv", name_feature,
               accumulation_train)
    write_data("../../../dataset/overlappingnums/covtype/mid/data_feature_val_"+str(i)+".csv", name_feature, accumulation_val)
    write_data("../../../dataset/overlappingnums/covtype/mid/data_feature_test_" + str(i) + ".csv", name_feature, accumulation_test)
