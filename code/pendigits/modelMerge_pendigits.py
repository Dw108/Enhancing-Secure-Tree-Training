...
# 读取数据
# 分裂属性的数据
# df = read_csv("../dataset/pendigits/pendigits_sub_feature_train.csv")
df = read_csv("../../dataset/pendigits/pendigits_sub_feature_train_enc.csv")
data = df.values
X_train_feature = data[:, 1:19].astype(int)
X_train_feature_ = data[:, 0:1].astype(int)
y_train_feature = data[:, -1].astype(int)

# df = read_csv("../dataset/pendigits/pendigits_sub_feature_validate.csv")
df = read_csv("../../dataset/pendigits/pendigits_sub_feature_validate_enc.csv")
data = df.values
X_validate_feature = data[:, 1:19].astype(int)
X_validate_feature_ = data[:, 0:1].astype(int)
y_validate_feature = data[:, -1].astype(int)

# df = read_csv("../dataset/pendigits/pendigits_sub_feature_test.csv")
df = read_csv("../../dataset/pendigits/pendigits_sub_feature_test_enc.csv")
data = df.values
...
# 分裂条目方式的数据
# df = read_csv("../dataset/pendigits/pendigits_sub_item_train.csv")
df = read_csv("../../dataset/pendigits/pendigits_sub_item_train_enc.csv")
data = df.values
X_train_item = data[:, 1:17]
X_train_item_ = data[:, 0:1]
y_train_item = data[:, -1]

# df = read_csv("../dataset/pendigits/pendigits_sub_item_validate.csv")
df = read_csv("../../dataset/pendigits/pendigits_sub_item_validate_enc.csv")
data = df.values
X_validate_item = data[:, 1:17]
X_validate_item_ = data[:, 0:1]
y_validate_item = data[:, -1]

# df = read_csv("../dataset/pendigits/pendigits_sub_item_test.csv")
df = read_csv("../../dataset/pendigits/pendigits_sub_item_test_enc.csv")
data = df.values
...
for ep in range(max_epochs):
    ...
    for i in range(num_iteration):
        ...
        model.add(K.layers.Dense(units=20, input_dim=20, kernel_initializer=init, activation='relu'))
