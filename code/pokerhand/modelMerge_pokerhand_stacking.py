...
# 按id将数据分割成n份，把id 分成5个set，按set划分
...
for i in range(accumulation_feature.shape[0]):
    ...
    tmp = numpy.concatenate((arr1, arr2, arr3, arr4, arr5, arr6), axis=0)
...
for ep in range(max_epochs):
    ...
    for i in range(num_iteration):
        ...
        # units 是输出大小
        model.add(K.layers.Dense(units=30, input_dim=60, activation='relu'))
