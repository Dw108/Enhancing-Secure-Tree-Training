...
df = read_csv("../../dataset/bias/"+name_file+"/"+name_file+"_sub_feature_train" + enc + scale + ".csv")
data = df.values
X_train_single_1 = np.concatenate((data[:, 1:2].astype(float), data[:, 3:4].astype(float), data[:, 5:13].astype(float)),
                                  axis=1)
X_train_single_2 = np.concatenate((data[:, 1:2].astype(float), data[:, 4:5].astype(float), data[:, 5:13].astype(float)),
                                  axis=1)
X_train_single_3 = np.concatenate((data[:, 2:3].astype(float), data[:, 3:4].astype(float), data[:, 5:13].astype(float)),
                                  axis=1)
X_train_single_4 = np.concatenate((data[:, 2:3].astype(float), data[:, 4:5].astype(float), data[:, 5:13].astype(float)),
                                  axis=1)
X_train_single_ = data[:, 0:1].astype(float)
...
y_train_single = data[:, -1].astype(float)
...
df = read_csv("../../dataset/bias/"+name_file+"/"+name_file+"_sub_feature_validate" + enc + scale + ".csv")
data = df.values
X_validate_single_1 = np.concatenate((data[:, 1:2].astype(float), data[:, 3:4].astype(float), data[:, 5:13].astype(float)),
                                     axis=1)
X_validate_single_2 = np.concatenate((data[:, 1:2].astype(float), data[:, 4:5].astype(float), data[:, 5:13].astype(float)),
                                     axis=1)
X_validate_single_3 = np.concatenate((data[:, 2:3].astype(float), data[:, 3:4].astype(float), data[:, 5:13].astype(float)),
                                     axis=1)
X_validate_single_4 = np.concatenate((data[:, 2:3].astype(float), data[:, 4:5].astype(float), data[:, 5:13].astype(float)),
                                     axis=1)
X_validate_single_ = data[:, 0:1].astype(float)
...
y_validate_single = data[:, -1].astype(float)
...
df = read_csv("../../dataset/bias/"+name_file+"/"+name_file+"_sub_feature_test" + enc + scale + ".csv")
data = df.values
X_test_single_1 = np.concatenate((data[:, 1:2].astype(float), data[:, 3:4].astype(float), data[:, 5:13].astype(float)),
                                 axis=1)
X_test_single_2 = np.concatenate((data[:, 1:2].astype(float), data[:, 4:5].astype(float), data[:, 5:13].astype(float)),
                                 axis=1)
X_test_single_3 = np.concatenate((data[:, 2:3].astype(float), data[:, 3:4].astype(float), data[:, 5:13].astype(float)),
                                 axis=1)
X_test_single_4 = np.concatenate((data[:, 2:3].astype(float), data[:, 4:5].astype(float), data[:, 5:13].astype(float)),
                                 axis=1)
X_test_single_ = data[:, 0:1].astype(float)
...
y_test_single = data[:, -1].astype(float)
...
bst_feature = XGBClassifier(nthread=1, n_estimators=n_estimators, max_depth=max_depth, learning_rate=1, objective='multi:softprob')
for i in range(_K):
    if i == 1:
        time_end = time.time()  # 结束计时
        time_c = time_end - time_start1  # 运行所花时间
        print('time cost 1_1', time_c, 's')
    X_feature_train = np.concatenate((X_feature[0:share_size * i, :], X_feature[share_size * (i+1):share_size * 6, :]))
    X_feature_val = X_feature[share_size * i:share_size * (i+1)]
    y_feature_train = np.concatenate((y_feature[0:share_size * i], y_feature[share_size * (i+1):share_size * 6]))
    y_feature_val = y_feature[share_size * i:share_size * (i+1)]
...
y_item_test = y_item[share_size * 6:]
time_start = time.time()
bst_item = XGBClassifier(nthread=1, n_estimators=n_estimators, max_depth=max_depth, learning_rate=1, objective='multi:softprob')
for i in range(_K):
# 分裂条目的训练
    X_item_train = np.concatenate((X_item[0:share_size * i, :], X_item[share_size * (i+1):share_size * 6, :]))
    X_item_val = X_item[share_size * i:share_size * (i+1)]
    y_item_train = np.concatenate((y_item[0:share_size * i], y_item[share_size * (i+1):share_size * 6]))
    y_item_val = y_item[share_size * i:share_size * (i+1)]
...
y_single1_test = y_single1[share_size * 6:]
time_start = time.time()
bst_single1 = XGBClassifier(nthread=1, n_estimators=n_estimators, max_depth=max_depth, learning_rate=1, objective='multi:softprob')
for i in range(_K):
# single1
    X_single1_train = np.concatenate((X_single1[0:share_size * i, :], X_single1[share_size * (i+1):share_size * 6, :]))
    X_single1_val = X_single1[share_size * i:share_size * (i+1)]
    y_single1_train = np.concatenate((y_single1[0:share_size * i], y_single1[share_size * (i+1):share_size * 6]))
    y_single1_val = y_single1[share_size * i:share_size * (i+1)]
...
y_single2_test = y_single2[share_size * 6:]
time_start = time.time()
bst_single2 = XGBClassifier(nthread=1, n_estimators=n_estimators, max_depth=max_depth, learning_rate=1, objective='multi:softprob')
for i in range(_K):
# single2
    X_single2_train = np.concatenate((X_single2[0:share_size * i, :], X_single2[share_size * (i+1):share_size * 6, :]))
    X_single2_val = X_single2[share_size * i:share_size * (i+1)]
    y_single2_train = np.concatenate((y_single2[0:share_size * i], y_single2[share_size * (i+1):share_size * 6]))
    y_single2_val = y_single2[share_size * i:share_size * (i+1)]
...
for i in range(_K):
# single3
    X_single3_train = np.concatenate((X_single3[0:share_size * i, :], X_single3[share_size * (i+1):share_size * 6, :]))
    X_single3_val = X_single3[share_size * i:share_size * (i+1)]
    y_single3_train = np.concatenate((y_single3[0:share_size * i], y_single3[share_size * (i+1):share_size * 6]))
    y_single3_val = y_single3[share_size * i:share_size * (i+1)]
    bst_single3 = XGBClassifier(nthread=1, n_estimators=n_estimators, max_depth=max_depth, learning_rate=1, objective='multi:softprob')
...
for i in range(_K):
    # single4
    X_single4_train = np.concatenate((X_single4[0:share_size * i, :], X_single4[share_size * (i + 1):share_size * 6, :]))
    X_single4_val = X_single4[share_size * i:share_size * (i + 1)]
    y_single4_train = np.concatenate((y_single4[0:share_size * i], y_single4[share_size * (i + 1):share_size * 6]))
    y_single4_val = y_single4[share_size * i:share_size * (i + 1)]
    bst_single4 = XGBClassifier(nthread=1, n_estimators=n_estimators, max_depth=max_depth, learning_rate=1, objective='multi:softprob')
...
for ep in range(max_epochs-1, max_epochs):
    ...
    for i in range(num_iteration):
        ...
        # units 是输出大小
        model.add(K.layers.Dense(units=30, input_dim=10, activation='relu'))
